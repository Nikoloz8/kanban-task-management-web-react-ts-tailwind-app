...
export default function EmptyBoard() {
    ...
    return paramsBoard?.columns.length! > 0 ?
        <div onClick={() => {
            setShowEditBoard(true)
            setColumns(paramsBoard?.columns!)
        }} className={`min-w-[280px]! h-[100vh] mt-[34px] cursor-pointer bg-[rgba(43,44,55,0.5)] rounded-[6px] ${H1} text-[#828FA3] hover:text-[#635FC7] flex items-center justify-center ${toggle ? "bg-[rgba(233,239,250,0.5)]" : ""}`}>+ New Column</div> : <div className={`flex h-[100%] w-[100%] items-center justify-center transition-all duration-1000`}>
