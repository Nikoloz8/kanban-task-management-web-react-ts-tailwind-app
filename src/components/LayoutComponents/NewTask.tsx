...
export default function NewTask() {
    ...
    if (!ctx) return
    const { handleSubmit, showAddTask, onSubmit, register, renderInputsArr, setRenderInputsArr, setShowStatus, showStatus, setStatus, status, paramsBoard, setShowDetails, setBoards, boards, setShowDotMenu, setShowAddTask, watch, reset, unregister, toggle } = ctx
    ...
    return (
        <div className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#2B2C37] p-[32px] flex flex-col gap-[24px] z-40 rounded-[6px] max-sm:w-[343px] w-[480px] ${!showAddTask ? "hidden" : ''} ${toggle && "bg-[#FFFFFF]"}`}>
            <h2 className={`${H2} text-[#FFFFFF] ${toggle && "text-[#000112]!"}`}>Add New Task</h2>
            <form onSubmit={handleSubmit(onSubmit)} action="" className="flex flex-col gap-[24px]">
                <div className="flex flex-col gap-[8px]">
                    ...
                    <input {...register("newTaskTitle")} type="text" id="title" className={`${inputStyle}   ${toggle && "placeholder:text-[rgba(0,1,18,0.25)]! text-[#000112]!"}}`} placeholder="e.g. Take coffee break" />
                </div>
                <div className="flex flex-col gap-[8px]">
                    <label htmlFor="description" className={`${H4} text-[#FFFFFF] ${toggle && "text-[#828FA3]!"}`}>Description</label>
                    <textarea {...register("newTaskDescription")} id="description" className={`${inputStyle} h-[112px]!   ${toggle && "placeholder:text-[rgba(0,1,18,0.25)]! text-[#000112]!"}`} placeholder="e.g. It’s always good to take a break. This 15 minute break will 
recharge the batteries a little." />
                ...
                <div className="flex flex-col gap-[8px]">
                    ...
                    <div className="flex flex-col gap-[12px]">
                        <div className="flex flex-col gap-[12px] overflow-y-auto max-h-[20vh]!">
                            {renderInputsArr.map((_e, i) => {
                                return <div key={i} className="flex gap-[16px] items-center">
                                    <input {...register(`newTaskSubtask${i}`)} className={`${inputStyle} ${toggle && "placeholder:text-[rgba(0,1,18,0.25)]! text-[#000112]!"}`} placeholder="e.g. Make coffee" type="text" />
                        ...
                        <button onClick={() => renderInputsArr.length > 0 ? setRenderInputsArr([...renderInputsArr, renderInputsArr[renderInputsArr.length - 1] + 1]) : setRenderInputsArr([0])} className={`w-[100%] p-[8px] text-center rounded-[20px] bg-[#FFFFFF] ${P1} font-[700] text-[#635FC7] cursor-pointer ${toggle && "bg-[rgba(99,95,199,0.1)]"}`}>+ Add New Subtask</button>
                    </div>
                </div>
            </form>
