...
export default function NewBoard() {
    ...
    return (
        <div className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#2B2C37] p-[32px] flex flex-col gap-[24px] z-40 rounded-[6px] max-sm:w-[343px] w-[480px] ${!showAddNewBoard && "hidden!"}`}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} action="" className="flex flex-col gap-[24px]">
                <div className="flex flex-col gap-[8px]">
                    ...
                    <input {...register("boardNameNew")} type="text" id="boardName" className={`${inputStyle}`} placeholder="e.g. Web Design" />
                </div>
                <div className="flex flex-col gap-[8px]">
                        ...
                        <div className="flex flex-col gap-[12px] overflow-y-auto max-h-[20vh]!">
                            {renderInputsArr.map((e, i) => {
                                ...
                                return <div key={i} className="flex gap-[16px] items-center">
                                    <input {...register(`columnNew${e}`)} className={`${inputStyle}`} placeholder="e.g. Todo" type="text" />
