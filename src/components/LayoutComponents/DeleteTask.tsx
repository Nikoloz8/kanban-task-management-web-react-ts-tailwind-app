...
export default function DeleteTask() {
    ...
    if (!ctx) return
    const { deleteTask, setDeleteTask, paramsBoard, setBoards, watch, boards, toggle } = ctx
    const { handleDeleteTask } = index({ paramsBoard, setBoards, watch, boards })
    return (
        <div className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#2B2C37] p-[32px] flex flex-col gap-[24px] z-40 rounded-[6px] max-sm:w-[343px] w-[480px] ${!deleteTask && "hidden"} ${toggle && "bg-[#FFFFFF]"}`}>
